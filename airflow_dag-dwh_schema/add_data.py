...
def createDB(dwhName: str) -> None:
    ...
    """
    conn, cur = DBConnect()
    cur.execute(f"""CREATE DATABASE IF NOT EXISTS {dwhName}
...
def read_stations_data(spark: SparkSession,
  data_location: str = 'data/*.csv') -> pd.DataFrame:
    df = spark.read.csv(data_location, header='true')
    return df


def preprocess_stations(path: str) -> pd.DataFrame:
...
def insert_to_dimRichardsStation(dwhName: str, data_path: str,
                                 table_name: str) -> None:
    """
    ...
    for _, row in df.iterrows():
        sqlQuery = f"""INSERT INTO {table_name} (timestamp_id, flow1, occupancy1,
                                flow2, occupancy2, flow3, occupancy3,
                                totalflow, weekday, hour, minute, second)
             VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"""
        data = (str(row[0]), float(row[1]), float(row[2]),
...
def insert_to_dimStationSummaryAirflow_table(dwhName: str, data_path: str,
                                    table_name: str) -> None:
    """
...
if __name__ == "__main__":
    dwhName = "Sensor_DW"
    dimName = "dimRichardStation"
    createDB(dwhName=dwhName)
    createTables(dwhName=dwhName, schema_name='richards_station_schema.sql')
    ...
    df = preprocess_richards_stations("../data_store/richards.csv")

    insert_to_dimRichardsStation(dwhName, '../data_store/richards.csv',
                                 dimName)
